...
from Ducknet2.ModelArchitecture.attentions import SCSA
...
class BasicBlockSCSA(BaseModule):
    """BasicBlock for ResNet + SCSA
    ...
    def __init__(self,
        ...
        super(BasicBlockSCSA, self).__init__(init_cfg=init_cfg)
        ...
        self.attn = self.attn_func(self.mid_channels, **scsa_cfg)
...
class BottleneckSCSA(BaseModule):
    """Bottleneck block for ResNet + SCSA
    ...
    def __init__(self,
        ...
        super(BottleneckSCSA, self).__init__(init_cfg=init_cfg)
...
def get_expansion(block, expansion=None):
    ...
    if isinstance(expansion, int):
        ...
        elif issubclass(block, BasicBlockSCSA):
            expansion = 1
        elif issubclass(block, BottleneckSCSA):
            expansion = 4
        else:
...
class ResLayer(nn.Sequential):
    ...
    Args:
        ...
        drop_path_rate (float or list): stochastic depth rate.
            Default: 0.
        scsa_cfg (dict): Config for SCSA. Default: None.
    """
...
class ResNetSCSA(BaseBackbone):
    """ResNet backbone + SCSA attention

    ResNet refer to the `paper <https://arxiv.org/abs/1512.03385>`__ for
    details.
    ...
    arch_settings = {
        18: (BasicBlockSCSA, (2, 2, 2, 2)),
        34: (BasicBlockSCSA, (3, 4, 6, 3)),
        50: (BottleneckSCSA, (3, 4, 6, 3)),
        101: (BottleneckSCSA, (3, 4, 23, 3)),
        152: (BottleneckSCSA, (3, 8, 36, 3))
    }
    ...
    def __init__(self,
        ...
        super(ResNetSCSA, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(ResNetSCSA, self).init_weights()
        ...
        if self.zero_init_residual:
            for m in self.modules():
                if isinstance(m, BottleneckSCSA):
                    constant_init(m.norm3, 0)
                elif isinstance(m, BasicBlockSCSA):
    ...
    def train(self, mode=True):
        super(ResNetSCSA, self).train(mode)
        self._freeze_stages()
